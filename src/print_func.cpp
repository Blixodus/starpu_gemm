...
void print_cpu_func(void * buffers[], void * cl_args) {
  char c;
  starpu_codelet_unpack_args(cl_args, &c);
...
#ifdef USE_CUDA
template <typename DataType>
__global__ void print_kernel(DataType *mat, int rows, int cols, int ld, char c) {
...
void print_cuda_func(void * buffers[], void * cl_args) {
  ...
  print_kernel<<<numBlocks, threadsPerBlock, 0, starpu_cuda_get_local_stream()>>>(mat, rows, cols, ld, c);
}
